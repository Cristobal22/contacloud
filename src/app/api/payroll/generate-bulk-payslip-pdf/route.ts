...
// ==========================================================================
// Security & Access Control (Copied from a working endpoint for consistency)
// ==========================================================================
...
// This is the same function used for single payslip generation.
// ==========================================================================
...
export async function POST(request: Request) {
    try {
        ...
        if (!companyId || !year || !month) {
            return new NextResponse(JSON.stringify({ error: 'Missing required parameters: companyId, year, month' }), { status: 400 });
        }
        ...
        const companyDocPromise = db.collection('companies').doc(companyId).get();
        
        const payrollsQuery = db.collection(`companies/${companyId}/payrolls`).where("period", "==", periodDateUTC);
        const payrollsSnapshotPromise = payrollsQuery.get();

        const [companyDoc, payrollsSnapshot] = await Promise.all([companyDocPromise, payrollsSnapshotPromise]);

        if (!companyDoc.exists) {
            return new NextResponse(JSON.stringify({ error: 'Company not found' }), { status: 404 });
        }
        if (payrollsSnapshot.empty) {
            return new NextResponse(JSON.stringify({ error: 'No processed payrolls found for the selected period' }), { status: 404 });
        }
        ...
        const employeeDocs = await db.getAll(...employeeIds.map(id => db.doc(`companies/${companyId}/employees/${id}`)));
        const employees = employeeDocs.map(doc => doc.data() as Employee);
        const employeesById = Object.fromEntries(employees.map(e => [e.id, e]));

        // 2. Create the master PDF
        const mergedPdf = await PDFDocument.create();

        // 3. Generate and merge each payslip
        for (const payroll of payrolls) {
            const employee = employeesById[payroll.employeeId];
            if (employee) {
                const payslipBytes = await generatePayslip(company, employee, payroll);
                const payslipPdf = await PDFDocument.load(payslipBytes);
                const [payslipPage] = await mergedPdf.copyPages(payslipPdf, [0]);
                mergedPdf.addPage(payslipPage);
            }
        }
        ...
        headers.set('Content-Disposition', `attachment; filename="Liquidaciones_${months[month - 1]}_${year}.pdf"`);

        return new NextResponse(mergedPdfBytes, { headers });

    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown server error';
        console.error('[API|generate-bulk-payslip-pdf] Error:', errorMessage, error);
