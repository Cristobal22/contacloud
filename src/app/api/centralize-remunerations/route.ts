...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Standardized to use 'companies' collection
        const payrollsSnapshot = await firestore.collection('companies').doc(companyId).collection('payrolls').where('period', '==', period).where('isCentralized', '!=', true).get();
        ...
        // Standardized to use 'companies' collection
        const voucherRef = firestore.collection('companies').doc(companyId).collection('vouchers').doc();
        ...
        payrolls.forEach(payroll => {
            // Standardized to use 'companies' collection
            const docRef = firestore.collection('companies').doc(companyId).collection('payrolls').doc(payroll.id);
